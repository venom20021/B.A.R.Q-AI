...
// ─── Voice Waveform (isolated — polls /voice/mic-level internally) ──

function VoiceWaveform({ isListening }: { isListening: boolean }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const fetchLevel = async () => {
      try {
        ...
        if (!mounted) return
        if (resp && typeof resp === 'object') {
          setLevel((resp as { level?: number }).level ?? 0)
        }
      } catch { /* stay at previous level */ }
    }
    fetchLevel()
    const interval = setInterval(fetchLevel, 200)
    return () => { mounted = false; clearInterval(interval) }
  }, [])
    ...
    const fetchVoiceStatus = async () => {
      try {
        ...
        if (resp && typeof resp === 'object') {
          ...
          setVoiceListening(v.is_listening ?? v.data?.is_listening ?? false)
        }
      } catch { /* backend unavailable */ }
    }
    fetchVoiceStatus()
  ...
  return (
    <div className="h-full w-full bg-black text-zinc-200 overflow-hidden relative font-mono">
      ...
      <div
        ...
        style={{
          backgroundImage: `
            linear-gradient(rgba(var(--a500-rgb), 0.06) 1px, transparent 1px),
            linear-gradient(90deg, rgba(var(--a500-rgb), 0.06) 1px, transparent 1px)
          `,
      ...
      <div className="relative z-10 h-full flex flex-col p-4 gap-3">
        ...
        <div className="flex items-center justify-between px-1 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-mono text-zinc-500 tracking-[0.15em]">
          ...
          <div className="flex items-center gap-2 px-3 py-1 rounded-lg bg-zinc-950/60 border border-zinc-800">
            <div className="relative flex items-center gap-1.5">
              ...
              <div className="flex flex-col">
                <span className={`text-[9px] font-mono tracking-wider uppercase transition-all duration-300 ${
                  voiceListening ? 'text-emerald-400 font-bold' : 'text-zinc-500'
                }`}>
                  {voiceListening ? 'ON AIR' : 'STANDBY'}
                </span>
                {/* Transient transcript toast */}
                {displayTranscript && (
                  <motion.span
            ...
            <VoiceWaveform isListening={voiceListening} />
          </div>
        ...
        />
            ...
            <div className="relative flex items-center justify-center">
              <motion.div
                className="absolute w-[340px] h-[340px] rounded-full border-0"
                style={{
              ...
              />
              <motion.div
                className="absolute w-[320px] h-[320px] rounded-full border-0"
                style={{
              ...
              <Suspense fallback={
                ...
                <ParticleSphere3D activeTheme={accent} isSpeaking={voiceListening} showGrid />
              </Suspense>
            </div>
            ...
            <div className="text-center mt-2">
              ...
              <div className="flex items-center justify-center gap-3 mt-1.5">
                <span className="text-[10px] font-mono text-zinc-500 tracking-[0.1em]">
            ...
            <div className="flex items-center justify-center gap-4 mt-2.5">
              {[
                ...
                <div key={mod.name} className="flex items-center gap-1.5">
                  ...
                  />
                  <span className={`text-[8px] font-mono tracking-wider ${mod.active ? 'text-purple-400/50' : 'text-zinc-600'}`}>
                    {mod.name}
                  </span>
                </div>
              ))}
          ...
          <div className="flex flex-col gap-2 min-h-0">
            ...
            <div className="flex items-center justify-between shrink-0">
              ...
              <span className="text-[8px] font-mono text-zinc-500 tabular-nums">{liveCount > 0 ? `${liveCount} new` : ''}</span>
            </div>
                        ...
                        return (
                          <>
                            <div className="flex items-baseline gap-1.5">
                              ...
                              <span className="text-[9px] font-mono text-zinc-500 uppercase">{w.city}</span>
                            </div>
                            <p className="text-[10px] font-mono text-zinc-400 mt-0.5 capitalize">{w.description}</p>
                            <div className="flex items-center gap-2 mt-1 text-[8px] font-mono text-zinc-500">
                            ...
                            {expandedBlock === 'weather' && (
                              ...
                              >
                                <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 text-[7px] font-mono">
                                  ...
                                  <span className="text-zinc-600">Feels like</span>
                                  <span className="text-zinc-400 text-right">{Math.round(w.feels_like_c)}°C</span>
                                </div>
                                <p className="text-[6px] text-zinc-700 text-center mt-1">Click to collapse</p>
                              </motion.div>
                            )}
                          </>
                        )
                        ...
                        return (
                          <>
                            <div className="flex items-baseline gap-1.5">
                              ...
                              <span className="text-[9px] font-mono text-zinc-500 uppercase">{latestFeed.label}</span>
                            </div>
                            ...
                            <p className="text-[8px] font-mono text-zinc-500 mt-1 truncate">{latestFeed.detail}</p>
                          </>
                        )
                      }
                      return (
                    ...
                    {(() => {
                      ...
                      return items.map(entry => (
                        <div key={entry.id} className="text-[7px] font-mono leading-[1.3]">
                          ...
                          <p className={expandedBlock === 'chat' ? 'text-zinc-500' : 'text-zinc-500 truncate'}>{entry.message}</p>
                          {entry.detail && <p className="text-zinc-600 truncate">{entry.detail}</p>}
                        </div>
                      ))
                    })()}
                    {activities.filter(a => a.type === 'chat').length === 0 && (
                      <div className="flex items-center justify-center h-full">
                        <span className="text-[9px] text-zinc-600">No messages</span>
                      </div>
                    )}
                    {/* Expand indicator */}
                    {expandedBlock === 'chat' && activities.filter(a => a.type === 'chat').length > 5 && (
                      <p className="text-[6px] text-zinc-700 text-center pt-1">Click to collapse</p>
                    )}
                    {expandedBlock !== 'chat' && activities.filter(a => a.type === 'chat').length > 5 && (
                      <p className="text-[6px] text-zinc-700 text-center pt-0.5">+{activities.filter(a => a.type === 'chat').length - 5} more &middot; Click to expand</p>
                    )}
                  </div>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto space-y-1 min-h-0">
                      ...
                      return items.map(entry => {
                        ...
                        return (
                          <div key={entry.id} className={`text-[7px] font-mono leading-[1.3]`}>
                            ...
                            <p className={expandedBlock === 'voice' && isLatest ? 'text-zinc-300' : expandedBlock === 'voice' ? 'text-zinc-500' : 'text-zinc-500 truncate'}>{entry.message}</p>
                            {entry.detail && <p className="text-zinc-600 truncate">{entry.detail}</p>}
                          </div>
                        )
                      })
                    ...
                    {activities.filter(a => a.type === 'voice').length === 0 && (
                      <div className="flex items-center justify-center h-full">
                        <span className="text-[9px] text-zinc-600">No commands</span>
                      </div>
                    )}
                    ...
                    {expandedBlock !== 'voice' && activities.filter(a => a.type === 'voice').length > 5 && (
                      <p className="text-[6px] text-zinc-700 text-center pt-0.5">+{activities.filter(a => a.type === 'voice').length - 5} more &middot; Click to expand</p>
                    )}
                  </div>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto space-y-1 min-h-0">
                    {(() => {
                      ...
                      return items.map(entry => (
                        <div key={entry.id} className="text-[7px] font-mono leading-[1.3]">
                          ...
                          <p className={expandedBlock === 'system' ? 'text-zinc-500' : 'text-zinc-500 truncate'}>{entry.message}</p>
                          {entry.detail && <p className="text-zinc-600 truncate">{entry.detail}</p>}
                        </div>
                      ))
                    })()}
                    {activities.filter(a => a.type === 'system').length === 0 && (
                      <div className="flex items-center justify-center h-full">
                        <span className="text-[9px] text-zinc-600">No events</span>
                      </div>
                    )}
                    ...
                    {expandedBlock !== 'system' && activities.filter(a => a.type === 'system').length > 5 && (
                      <p className="text-[6px] text-zinc-700 text-center pt-0.5">+{activities.filter(a => a.type === 'system').length - 5} more &middot; Click to expand</p>
                    )}
                  </div>
                </motion.div>
              )}
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-0.5">
                {threads.map((thread) => (
                  <div key={thread.name} className="flex items-center gap-1 group">
                    ...
                    />
                    <span className="text-[7px] font-mono text-zinc-500 truncate group-hover:text-zinc-400 transition-colors">
                      {thread.name}
                    </span>
                  </div>
                ))}
